...
However, it turns out that the same problem has returned. You see, I
also don't like the idea of having a level (like "Current assets")
which "contains" a mixture of accounts and other levels. I think
levels should either split into sublevels (those sublevels providing a
/complete/ decomposition of the level) or contain accounts. I don't
want to have the alternative because then you end up with
